...
- Tool policy, risk ceiling and safety budget (Slice 7, sections 16-18): `request_engine.agent_policies` (PK `(organization_id, agent_principal_id)`, tenant-isolated RLS, `allowed_capabilities`/`denied_capabilities` arrays, `risk_ceiling`, `max_mutations_per_minute`, revisioned with provenance) plus `request_engine.agent_budget_windows` and the SECURITY DEFINER `request_engine.upsert_agent_policy` (migration `0026_agent_policy`). Enforcement lives in `AgentPolicyActorResolver` (`platform/security/agent_policy_http.py`), wrapped around delegated resolution inside `create_native_app`: absence of a policy denies every agent operation (fail closed), routes without capability metadata deny agents, effective authority is granted ∩ allowed − denied read fresh per request, risk above the ceiling denies (`agent_risk_denied`), AUTHORITY_CHANGE is unconditionally banned in code for agent actors, and the per-minute mutation budget is consumed through a race-safe conditional upsert (`429 agent_budget_exceeded`, reads unaffected). Policy is managed through `GET/PUT /v1/agents/{id}/policy` (`agent_policy_read`/`agent_policy_replace`) under `agent.policy.read`/`agent.manage_policy`. Day-one deferrals: approval states (`REQUESTER_CONFIRMATION`/`HUMAN_APPROVAL`/`PRIVILEGED_APPROVAL`) require an approval product surface and are not modeled yet — operations above the ceiling are denied for agents, fail-closed; per-task budgets and financial-amount budgets are deferred until task identity is defined.

Proof ownership: `tests/db/test_agent_policy.py` (tool/risk policy mechanics, SQLSTATE guards, tenant isolation and the deterministic budget-window race), `tests/db/test_agent_governance.py` (six real-PostgreSQL proofs), `tests/db/test_workload_authentication.py`, `tests/e2e/test_native_agent_lifecycle.py` (E2E B analog including policy ceiling proofs), `tests/e2e/test_native_agent_safety_enforcement.py` (budget exhaustion E2E), `tests/e2e/test_platform_provisioning_journey.py` (bootstrap via the `platform_bootstrap_cli` env-driven path: tenant provisioner → organization from zero → staff lifecycle → agent lifecycle with policy assignment, including no-amplification and provisioner-cannot-access-tenant negatives), and unit evidence in `tests/unit/test_agent_governance_*.py`, `tests/unit/test_agent_policy_*.py`, `tests/unit/test_workload_auth.py` and `tests/unit/test_authority_plane_registry.py`.

Still open in Slice 7: risk approval states (REQUESTER_CONFIRMATION/HUMAN_APPROVAL/PRIVILEGED_APPROVAL), which require an approval product surface. Slice 8 (delegated task authority with actor/subject attribution, sections 13-15) and Slices 9-12 (provider conformance, external B2B providers, optional MCP projection, adversarial closure) are open.

Single-app composition (owner decision, current branch): the canonical production app is a SINGLE composition root — `create_native_app` (`entrypoints/http/app.py` via `create_app`) — which now also mounts the operational configuration surfaces (`/v1/operations/*` locations/hours/holidays/terms/assignments/exceptions, discovery operational, tenancy operational profile/contacts) through `install_operational_modules`. Authorization is unchanged: the operational installers share the same execution actor resolver chain as the business modules (acting-operator relay, tenant capability filtering, correlation binding), and the EXISTING capability metadata plus granular Representation grant/revoke continues to decide who reaches what. `create_operational_app` remains available as the operator-only subset composition for legacy/e2e compositions; it no longer represents the production shape.