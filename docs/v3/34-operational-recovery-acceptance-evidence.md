...
The code-bearing acceptance baseline above is historical provenance; GitHub's required checks on the actual PR head are the authoritative exact-head merge gate. For the current tranche (`feature/f5-roadmap-authoritative-recovery`, PR #83), the rows registered from this branch become merge evidence only when that PR head's required checks are green.
