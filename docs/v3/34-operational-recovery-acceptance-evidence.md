...
| Autonomous reschedule envelope (14/I) | `tests/e2e/test_f5_recovery_autonomous_reschedule.py` | PASS | Without a granted policy the scheduled handler creates no recovery actions, notifications or reservation movement (dormant by default). After an operator grants the envelope through the real `configure_autonomy` PUT surface (attribution recorded), the handler moves exactly the affected reservations whose proposal targets fit the envelope across assessment cycles, as durable `succeeded` RESCHEDULE actions under the `operational_recovery_automation` principal, with one deduped `operational_recovery_rescheduled` notification per moved subject and subjects/locations preserved. A tight envelope (`max_delay_minutes` below the candidate delay) keeps every candidate untouched and the incident open for the operator, and the per-incident budget durably caps autonomous attempts. |
