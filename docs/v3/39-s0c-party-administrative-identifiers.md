...
The write capability is intentionally not part of the documented bot grant subset. Front-desk
operators may receive it explicitly; a channel integration does not inherit it from
`parties.register` or `parties.lookup`.
...
- capability/grant isolation so the existing bot subset does not imply the write capability.
