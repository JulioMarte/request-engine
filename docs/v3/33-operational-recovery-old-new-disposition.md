...
| event-driven reprojection/escalation on material operational events | PARTIAL / REPROJECTION + POLICY EVALUATION DELIVERED | Authoritative source-table triggers durably advance the recovery source revision and schedule one deduped reassessment per revision; the handler commits incidents/proposals and evaluates escalation/communication policy only at current truth (contract §5). The evaluation records a durable immutable escalation outcome per incident and revision; delivering customer communication remains the explicit COMMUNICATE_IMPACT action. Change-storm coalescing policy and autonomous escalation remain separate requirements needing their own evidence (`10-worker-runtime-hardening.md`); the bounded fallback sweep is delivered on the sweep lane (worker stream that repairs lost wake-ups without resuscitating dead/cancelled actions). |
...
Accordingly, the current tranche should be described as the **F5 core slice plus the full recovery workflow tranche**, not as proof that the entire original recovery roadmap has been delivered. The bounded fallback sweep is delivered on the sweep lane; the remaining open rows — cross-Organization replacement, change-storm coalescing policy and autonomous escalation — remain roadmap debt and must be delivered or explicitly superseded before the broader product line can be called complete.
