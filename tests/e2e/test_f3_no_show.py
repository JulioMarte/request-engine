...
def _seed_called_entry(conn: PgConnection, sandbox: TenantSandbox) -> UUID:
    row = conn.execute(
        ...
        "(organization_id,service_queue_id,subject_party_id,status,arrived_at,admitted_at,called_at) "
...
async def test_mark_no_show_closes_called_entry_without_creating_service_session(
    e2e_admin_conn: PgConnection,
    e2e_session_factory: SessionFactory,
) -> None:
    sandbox = seed_tenant_sandbox(e2e_admin_conn, "f3-no-show")
    ...
    actor = ActorContext(
        ...
        capabilities=frozenset({"queue.mark_no_show"}),
    )
    ...
    assert e2e_admin_conn.execute(
        "SELECT status,revision,service_started_at,completed_at "
        "FROM request_engine.queue_entries WHERE id=%s",
        (entry_id,),
    ).fetchone() == ("no_show", 2, None, None)
    assert e2e_admin_conn.execute(
    ...
    assert e2e_admin_conn.execute(
        ...
        "WHERE organization_id=%s AND command_name='queue.mark_no_show' "
        "AND aggregate_id=%s",
        (sandbox.organization_id, entry_id),
    ).fetchone() == (1,)
    assert e2e_admin_conn.execute(
        "SELECT count(*) FROM request_engine.outbox_messages "
        "WHERE organization_id=%s AND event_type='queue.entry_no_show.v1' "
        "AND aggregate_id=%s",
        (sandbox.organization_id, entry_id),
    ).fetchone() == (1,)
