...
async def prepare_recovery_world(
    ...
    use_five_minute_slots(conn, sandbox)
    seed_today_schedule(conn, sandbox)
    scheduled_id, walk_id = await configure_projection(client, sandbox)
    reservations, slots = await book_commitments(client, conn, sandbox)
    replacement = seed_replacement_resource(conn, sandbox)
    restrict_source_to_first_six(conn, sandbox, slots)
