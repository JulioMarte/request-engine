...
async def test_f5_scheduled_assessment_records_newly_material_escalation_policy(
    ...
    assert outcome is not None and outcome.operator_escalation_required is True
...
async def test_f5_escalation_worsens_when_delay_becomes_capacity_shortfall(
    ...
    assert second.applied is True and second.incident is not None and second.escalation is not None
    ...
    assert automatic_recovery_facts(e2e_admin_conn, sandbox.organization_id) == (0, 1, 1)
    task2 = autonomous_impact_task(e2e_admin_conn, sandbox, second.incident.id, shortfall_revision)
