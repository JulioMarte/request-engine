...
from .f3_acceptance_assertions import capacity_claim_snapshot, reservation_snapshot, seed_walk_in_subject
from .f4_capacity_support import f4_actor, seed_today_schedule
from .f4_operational_day_support import book_two_same_day, call_and_start, configure_projection, read_projection
from .operational_support import PgConnection
from .tenant_sandbox import auth, client_with_actors, seed_tenant_sandbox

pytestmark = [pytest.mark.asyncio, pytest.mark.e2e, pytest.mark.postgres, pytest.mark.invariant,
              pytest.mark.contract, pytest.mark.adversarial, pytest.mark.capacity,
              pytest.mark.provenance, pytest.mark.temporal]
...
async def test_f4_operational_day_reprojects_planning_queue_and_service_truth(
    ...
    async with client_with_actors(e2e_session_factory, {sandbox.token: f4_actor(sandbox)}) as client:
        expected_id, walk_id = await configure_projection(client, sandbox)
        first_reservation, future_reservation = await book_two_same_day(client, e2e_admin_conn, sandbox)
        ...
        checked = await client.post(
            ...
            json={"subject_party_id": str(sandbox.party_id), "reservation_id": str(first_reservation),
        ...
        walk_in = await client.post(
            f"/v1/queues/{sandbox.queue_id}/check-in",
            json={"subject_party_id": str(seed_walk_in_subject(e2e_admin_conn, sandbox)),
