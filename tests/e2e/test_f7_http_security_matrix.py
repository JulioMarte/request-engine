...
_SIGNING_KEY = b"request-engine-f7-security-signing-key"
...
async def _request(client: AsyncClient, operation: PublicHttpOperation):
    headers = {"Idempotency-Key": f"f7-security-{uuid4().hex}"}
...
@pytest.mark.asyncio
@pytest.mark.parametrize("operation", F7_HTTP_OPERATIONS, ids=_id)
async def test_every_f7_operation_rejects_unauthenticated_without_mutation(
...
@pytest.mark.asyncio
@pytest.mark.parametrize("operation", F7_HTTP_OPERATIONS, ids=_id)
async def test_every_f7_operation_rejects_missing_capability_without_mutation(
    operation: PublicHttpOperation,
    e2e_admin_conn: support.PgConnection,
    e2e_session_factory: SessionFactory,
) -> None:
