...
def _offering_version_without_requirements(
    conn: PgConnection,
...
def _capacity_fixture(conn: PgConnection) -> tuple[UUID, UUID, UUID, UUID, UUID]:
    ...
    subject_party_id = _party(conn, organization_id)
    offering_version_id = _offering_version_without_requirements(conn, organization_id)
...
def test_zero_requirement_offering_cannot_commit_confirmed_reservation(
    ...
    subject_party_id = _party(admin_conn, organization_id)
    offering_version_id = _offering_version_without_requirements(admin_conn, organization_id)
...
@pytest.mark.postgres
def test_nonbooking_offering_version_may_have_zero_requirements(admin_conn: PgConnection) -> None:
    organization_id = _organization(admin_conn)
    version_id = _offering_version_without_requirements(
        admin_conn,
        organization_id,
        bookable=False,
    )
...
@pytest.mark.postgres
def test_reservation_closed_is_not_a_baseline_state(admin_conn: PgConnection) -> None:
    organization_id = _organization(admin_conn)
    subject_party_id = _party(admin_conn, organization_id)
    offering_version_id = _offering_version_without_requirements(admin_conn, organization_id)
...
def test_past_confirmed_reservation_does_not_freeze_resource_maintenance(
    ...
    organization_id, subject_id, version_id, requirement_id, resource_id = _capacity_fixture(
        admin_conn
    )
    _reservation_with_claim(
        ...
        during="[2020-01-01 10:00+00,2020-01-01 11:00+00)",
    )
...
def test_future_confirmed_reservation_still_blocks_resource_maintenance(
    ...
    _reservation_with_claim(
        ...
        requirement_id=requirement_id,
        resource_id=resource_id,
        during="[2099-01-01 10:00+00,2099-01-01 11:00+00)",
    )
