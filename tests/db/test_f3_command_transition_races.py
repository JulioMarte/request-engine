...
from f3_live_ops_race_support import (
...
def _one_winner(results: list[object], types: tuple[type, ...]) -> None:
    assert sum(isinstance(result, types) for result in results) == 1
...
async def test_start_service_vs_no_show_commands_have_one_effectful_winner(
    ...
    _one_winner(results, (ServiceSession, LiveQueueEntry))
    ...
    assert _effects(admin_conn, setup.organization_id, *winner) == (1, 1)
    assert _effects(admin_conn, setup.organization_id, *loser) == (0, 0)
...
async def test_pause_pause_commands_create_one_interruption_and_one_effect(
    ...
    _one_winner(results, (ServiceSession,))
...
async def test_session_vs_resource_activity_commands_have_one_effectful_winner(
    ...
    _one_winner(results, (ServiceSession, ResourceActivity))
    sessions = admin_conn.execute(
