...
def test_root_establishment_materializes_one_native_platform_controller_atomically(
    ...
    assert admin_conn.execute(
        ...
        (principal_id,),
    ).fetchone() == ("platform", "human", True, 11)
    assert admin_conn.execute(
    ...
    assert grants == [
        ...
        ("platform.provisioner.manage_lifecycle", False, "trust_bootstrap"),
        ("platform.provisioner.read", False, "trust_bootstrap"),
        ("platform.recovery_operator.provision", False, "trust_bootstrap"),
