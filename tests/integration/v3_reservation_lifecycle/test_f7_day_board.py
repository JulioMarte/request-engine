...
async def test_day_board_composes_eta_and_check_in_without_erasing_assertion(
    ...
    reader = PostgresReservationDayBoardReader(session_factory)

    def read_window(location_id: UUID | None = None, limit: int = 500):
    ...
    filtered = await read_window(location_id=uuid4())
    assert filtered == ()
    ...
    after = await read_window()

    assert after[0].attendance_outcome == "checked_in"
