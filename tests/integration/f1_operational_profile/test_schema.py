...
def _fixture(conn: PgConnection, label: str) -> Fixture:
    ...
    location_id = _uuid(
        conn,
        "INSERT INTO request_engine.locations (organization_id, location_key, display_name, timezone) VALUES (%s, %s, %s, 'America/Santo_Domingo') RETURNING id",
...
def _resource_revision(conn: PgConnection, resource_id: UUID) -> int:
    row = conn.execute(
...
def _location_revision(conn: PgConnection, location_id: UUID) -> int:
    row = conn.execute(
...
def test_f1_assignment_is_multi_location_tenant_safe_and_non_overlapping(
    ...
    with pytest.raises(psycopg.errors.ExclusionViolation):
        admin_conn.execute(
            ...
            """,
            (fixture.organization_id, fixture.resource_id, fixture.location_id),
        )

    location_two = _uuid(
        admin_conn,
        "INSERT INTO request_engine.locations (organization_id, location_key, display_name, timezone) VALUES (%s, %s, 'Second Location', 'America/Santo_Domingo') RETURNING id",
        (fixture.organization_id, f"second-{uuid4().hex}"),
    )
    assignment_two = _uuid(
        admin_conn,
        """
        INSERT INTO request_engine.resource_location_assignments (
            organization_id, resource_id, location_id, effective_during
        ) VALUES (%s, %s, %s, tstzrange('2030-01-01T00:00:00+00'::timestamptz, NULL, '[)'))
        RETURNING id
        """,
        (fixture.organization_id, fixture.resource_id, location_two),
    )
    assert assignment_two != fixture.assignment_id
    ...
    with pytest.raises(psycopg.errors.ForeignKeyViolation):
        admin_conn.execute(
            ...
            INSERT INTO request_engine.resource_location_assignments (
                organization_id, resource_id, location_id, effective_during
            ) VALUES (%s, %s, %s, tstzrange('2040-01-01T00:00:00+00'::timestamptz, NULL, '[)'))
            """,
            (fixture.organization_id, fixture.resource_id, foreign.location_id),
        )

    with pytest.raises(psycopg.errors.CheckViolation):
        admin_conn.execute(
            "INSERT INTO request_engine.locations (organization_id, location_key, display_name, timezone, latitude) VALUES (%s, %s, 'Bad Geo', 'UTC', 18.5)",
            (fixture.organization_id, f"bad-geo-{uuid4().hex}"),
        )


@pytest.mark.postgres
def test_f1_material_schedule_children_change_observation_tokens(admin_conn: PgConnection) -> None:
    fixture = _fixture(admin_conn, "tokens")

    location_before = _location_revision(admin_conn, fixture.location_id)
    admin_conn.execute(
        "INSERT INTO request_engine.location_operational_hours (organization_id, location_id, weekday, local_start, local_end) VALUES (%s, %s, 0, '08:00', '17:00')",
        (fixture.organization_id, fixture.location_id),
    )
    location_after_hours = _location_revision(admin_conn, fixture.location_id)
    ...
    admin_conn.execute(
        ...
        ) VALUES (%s, %s, tstzrange('2030-05-06T12:00:00+00', '2030-05-06T13:00:00+00', '[)'), 'unavailable')
        """,
        (fixture.organization_id, fixture.location_id),
    )
    assert _location_revision(admin_conn, fixture.location_id) > location_after_hours

    resource_before = _resource_revision(admin_conn, fixture.resource_id)
    admin_conn.execute(
        "INSERT INTO request_engine.resource_location_availability (organization_id, resource_location_assignment_id, weekday, local_start, local_end) VALUES (%s, %s, 0, '09:00', '16:00')",
        (fixture.organization_id, fixture.assignment_id),
    )
    resource_after_hours = _resource_revision(admin_conn, fixture.resource_id)
    ...
    admin_conn.execute(
        ...
        ) VALUES (%s, %s, tstzrange('2030-05-06T14:00:00+00', '2030-05-06T15:00:00+00', '[)'), 'unavailable')
        """,
        (fixture.organization_id, fixture.assignment_id),
    )
    assert _resource_revision(admin_conn, fixture.resource_id) > resource_after_hours
...
def test_f1_context_terms_are_effective_dated_and_exactly_revisioned(
    ...
    _uuid(
        admin_conn,
        "INSERT INTO request_engine.offering_version_booking_terms (organization_id, offering_version_id, amount, currency) VALUES (%s, %s, 3500, 'DOP') RETURNING id",
    ...
    context_id = _uuid(
        ...
        INSERT INTO request_engine.booking_context_terms (
            ...
            %s, %s, %s,
            tstzrange('2030-01-01T00:00:00+00', '2031-01-01T00:00:00+00', '[)'),
            4000, 'DOP', 45
        ) RETURNING id
        """,
        (fixture.organization_id, fixture.assignment_id, fixture.offering_version_id),
    )
    ...
    with pytest.raises(psycopg.errors.ExclusionViolation):
        admin_conn.execute(
            ...
            INSERT INTO request_engine.booking_context_terms (
                organization_id, resource_location_assignment_id, offering_version_id,
                effective_during, amount, currency
            ) VALUES (%s, %s, %s, tstzrange('2030-06-01T00:00:00+00', '2030-07-01T00:00:00+00', '[)'), 4200, 'DOP')
            """,
            (fixture.organization_id, fixture.assignment_id, fixture.offering_version_id),
        )
    ...
    admin_conn.execute(
        ...
        INSERT INTO request_engine.booking_context_terms (
            organization_id, resource_location_assignment_id, offering_version_id,
            effective_during, amount, currency, active
        ) VALUES (%s, %s, %s, tstzrange('2030-06-01T00:00:00+00', '2030-07-01T00:00:00+00', '[)'), 4200, 'DOP', false)
        """,
        (fixture.organization_id, fixture.assignment_id, fixture.offering_version_id),
    )

    context_before_row = admin_conn.execute(
        "SELECT revision FROM request_engine.booking_context_terms WHERE id = %s",
        (context_id,),
    ).fetchone()
    assert context_before_row is not None
    context_before = cast(int, context_before_row[0])
    resource_before = _resource_revision(admin_conn, fixture.resource_id)

    admin_conn.execute(
        "UPDATE request_engine.booking_context_terms SET amount = 4100 WHERE organization_id = %s AND id = %s",
        (fixture.organization_id, context_id),
    )
    context_after_row = admin_conn.execute(
        "SELECT revision FROM request_engine.booking_context_terms WHERE id = %s",
        (context_id,),
    ).fetchone()
    assert context_after_row is not None
    assert cast(int, context_after_row[0]) == context_before + 1
    assert _resource_revision(admin_conn, fixture.resource_id) == resource_before
...
def test_f1_contextual_claim_uses_assignment_not_legacy_resource_location(
    ...
    # assignment -> Resource, rather than merely relying on a cross-tenant FK.
    other_resource_id = _uuid(
        admin_conn,
        "INSERT INTO request_engine.resources (organization_id, resource_key, display_name, capacity_model, capacity_units) VALUES (%s, %s, 'Other Resource', 'exclusive', 1) RETURNING id",
        (fixture.organization_id, f"other-resource-{uuid4().hex}"),
    )
    admin_conn.execute(
        "INSERT INTO request_engine.resource_capability_assignments (organization_id, resource_id, capability_id) VALUES (%s, %s, %s)",
        (fixture.organization_id, other_resource_id, fixture.capability_id),
    )
    other_assignment_id = _uuid(
        admin_conn,
        """
        INSERT INTO request_engine.resource_location_assignments (
            organization_id, resource_id, location_id, effective_during
        ) VALUES (%s, %s, %s, tstzrange('2030-01-01T00:00:00+00', NULL, '[)')) RETURNING id
        """,
    ...
    with pytest.raises(psycopg.errors.CheckViolation), admin_conn.transaction():
            hold_id = _uuid(
                ...
                INSERT INTO request_engine.capacity_holds (
                    ...
                    %s, %s, %s, %s,
                    tstzrange('2030-06-04T14:00:00+00', '2030-06-04T14:30:00+00', '[)'),
...
@pytest.mark.postgres
def test_f1_commercial_commitment_is_append_only(admin_conn: PgConnection) -> None:
    fixture = _fixture(admin_conn, "commitment")
    base_terms_id = _uuid(
        admin_conn,
        "INSERT INTO request_engine.offering_version_booking_terms (organization_id, offering_version_id, amount, currency) VALUES (%s, %s, 3500, 'DOP') RETURNING id",
        (fixture.organization_id, fixture.offering_version_id),
    ...
    with admin_conn.transaction():
        ...
        admin_conn.execute(
            ...
            INSERT INTO request_engine.capacity_claims (
                ...
                %s, %s, %s, %s, %s,
                tstzrange('2030-06-03T14:00:00+00', '2030-06-03T14:30:00+00', '[)'), 1
            )
            """,
            (
                fixture.organization_id,
        ...
        admin_conn.execute(
            ...
            """,
            (reservation_id, fixture.organization_id, base_terms_id, f"fp-{uuid4().hex}"),
        )

    row = admin_conn.execute(
        "SELECT amount, currency, planned_duration_minutes FROM request_engine.reservation_commercial_commitments WHERE reservation_id = %s",
        (reservation_id,),
    ).fetchone()
    assert row == (Decimal("3500.000000"), "DOP", 30)

    with pytest.raises(psycopg.Error) as immutable:
        admin_conn.execute(
            "UPDATE request_engine.reservation_commercial_commitments SET amount = 9999 WHERE reservation_id = %s",
            (reservation_id,),
        )
...
@pytest.mark.postgres
def test_f1_tables_force_rls_and_hide_foreign_tenant_rows(
    admin_conn: PgConnection,
    pg_conninfo: str,
) -> None:
    ...
    first_endpoint = _uuid(
        admin_conn,
        "INSERT INTO request_engine.organization_public_contact_endpoints (organization_id, channel, normalized_value) VALUES (%s, 'phone', %s) RETURNING id",
    ...
    try:
        ...
        visible = app_conn.execute(
            "SELECT id, organization_id FROM request_engine.organization_public_contact_endpoints"
        ).fetchall()
        assert visible == [(first_endpoint, first.organization_id)]

        with pytest.raises(psycopg.Error) as denied:
            app_conn.execute(
                "INSERT INTO request_engine.organization_public_contact_endpoints (organization_id, channel, normalized_value) VALUES (%s, 'email', %s)",
