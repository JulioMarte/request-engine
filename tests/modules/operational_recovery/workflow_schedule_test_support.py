...
class FakeWorkflowRepository:
    def __init__(self) -> None:
        self.incident = RecoveryIncident(
            INCIDENT, ORG, QUEUE, RESOURCE, LOCATION, RecoveryIncidentStatus.OPEN,
            RecoveryImpactKind.CAPACITY_SHORTFALL, 1, 3, "source:v1", None,
            NOW, NOW, None, 1,
        )
    ...
    async def prepare_action(self, **kwargs: object):
        ...
        self.action = RecoveryAction(
            ACTION, ORG, INCIDENT, kwargs["action_kind"], RecoveryActionStatus.PREPARED,
            PRINCIPAL, kwargs["idempotency_key"], kwargs["command_fingerprint"],
            kwargs["expected_source_revision"], kwargs["payload"], {}, None, NOW, None, None,
        )
        return self.action, True

    async def transition_action(self, *, status: RecoveryActionStatus,
        owner_steps: Mapping[str, object] | None = None, failure_code: str | None = None, **_: object):
...
class FakeSchedule:
    ...
    async def extend_assignment_hours(self, request: RecoveryAssignmentExtensionRequest):
        ...
        return RecoveryAssignmentExtensionResult(EXCEPTION, ASSIGNMENT, request.start_at, request.end_at, 12)


class FakeCapacity:
    async def assess_recovery_capacity(self, *, organization_id: UUID, service_queue_id: UUID):
        return RecoveryCapacityAssessment(
            QUEUE, RESOURCE, LOCATION, NOW, NOW + timedelta(hours=8), ProjectionState.KNOWN, (),
            7200, 3600, 0, 0, 0, "source:v4", {},
...
def command() -> ExtendRecoveryDayCommand:
    return ExtendRecoveryDayCommand(
        ORG, PRINCIPAL, AUTHORITY, INCIDENT, 3, ASSIGNMENT,
        NOW + timedelta(hours=4), NOW + timedelta(hours=6), 11,
        "extend-day-command", "recover same-day capacity",
    )
