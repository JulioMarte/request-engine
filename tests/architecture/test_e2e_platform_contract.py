...
def test_enabled_e2e_suites_have_reusable_registry_contract() -> None:
    ...
    for name, spec in enabled.items():
        ...
        assert spec["fresh_world"] is True, f"{name} must default to an isolated authoritative world"
...
def test_black_box_runner_image_cannot_install_application_shortcuts() -> None:
    ...
    forbidden = ("src/request_engine", "pip install", "uv sync", "psycopg", "sqlalchemy", "docker.sock")
    found = [token for token in forbidden if token in dockerfile]
    assert not found, f"black-box runner Dockerfile contains forbidden application/database shortcuts: {found}"
