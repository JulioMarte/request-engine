...
from tests.e2e.http_surface import PUBLIC_HTTP_OPERATIONS

_EXPECTED_OPERATIONS = (
    ("capabilities.list", "GET", "/v1/capabilities", None),
...
_EXPECTED_CAPABILITIES = (
    ("business.get_info", "public", "query", "none", "none", None, None, ("business.read",), True),
    (
    ...
    (
        ...
        None,
        None,
        ("booking.subject_override",),
        False,
    ),
    ("queue.list", "public", "query", "none", "none", None, None, ("queue.read",), True),
    (
        "queue.join",
        "public",
        "command",
    ...
    ("queue.subject_override", "operator", "command", "required", "none", None, None, (), False),
    (
        "waitlist.join",
        "public",
        "command",
...
_EXPECTED_DYNAMIC_ERROR_CODES = frozenset(
...
def _literal_error_codes(path: Path) -> set[str]:
    ...
    for node in ast.walk(tree):
        if not isinstance(node, ast.Call):
            continue
        func = node.func
        if not isinstance(func, ast.Name) or func.id != "ErrorBody":
            continue
...
def test_v3_public_http_operation_surface_is_frozen() -> None:
    actual = tuple(
        (operation.name, operation.method, operation.path_template, operation.capability)
...
def test_v3_capability_registry_is_frozen() -> None:
    actual = tuple(
        (
            ...
            definition.party_scope,
            definition.override_capability,
            tuple(sorted(definition.legacy_aliases)),
            definition.runtime_available,
        )
...
def test_v3_public_error_code_inventory_is_frozen() -> None:
    ...
    for code in _EXPECTED_DYNAMIC_ERROR_CODES:
        assert f'_conflict("{code}"' in request_errors


def test_v3_public_contract_inventory_has_no_unclassified_operation_capabilities() -> None:
    capability_keys = {definition.key for definition in CAPABILITIES}
    for operation in PUBLIC_HTTP_OPERATIONS:
        if operation.capability is None:
            assert operation.name == "capabilities.list"
            continue
        assert operation.capability in capability_keys
        definition = next(
            definition for definition in CAPABILITIES if definition.key == operation.capability
        )
        assert definition.runtime_available
        assert definition.exposure.value in {"public", "operator"}
        assert (definition.idempotency.value == "required") is operation.idempotency_required
