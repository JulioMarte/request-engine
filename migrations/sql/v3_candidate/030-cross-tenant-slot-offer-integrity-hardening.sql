...
-- Immediate creation locks close the validation/use race, but source aggregates
-- can also be mutated later. Enforce I39 on the transaction's final state: an
-- offer that still commits as `offered` must still reference an open
-- opportunity, active candidate and live Hold with matching booking provenance.
CREATE INDEX slot_offers_active_waitlist_source_idx
    ON request_engine.slot_offers (organization_id, waitlist_entry_id)
    WHERE status = 'offered';

CREATE FUNCTION request_engine.assert_offered_slot_offer_source_consistency(
    p_organization_id uuid,
    p_slot_offer_id uuid
)
RETURNS void
LANGUAGE plpgsql
SET search_path = pg_catalog, request_engine
...
CREATE FUNCTION request_engine.check_offered_slot_offer_source_consistency()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = pg_catalog, request_engine
AS $function$
...
CREATE CONSTRAINT TRIGGER slot_offers_source_consistency_deferred
AFTER INSERT OR UPDATE ON request_engine.slot_offers
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION request_engine.check_offered_slot_offer_source_consistency();

CREATE CONSTRAINT TRIGGER capacity_holds_slot_offer_consistency_deferred
AFTER UPDATE ON request_engine.capacity_holds
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION request_engine.check_offered_slot_offer_source_consistency();

CREATE CONSTRAINT TRIGGER waitlist_entries_slot_offer_consistency_deferred
AFTER UPDATE ON request_engine.waitlist_entries
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION request_engine.check_offered_slot_offer_source_consistency();

CREATE CONSTRAINT TRIGGER slot_opportunities_offer_consistency_deferred
AFTER UPDATE ON request_engine.slot_opportunities
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION request_engine.check_offered_slot_offer_source_consistency();
