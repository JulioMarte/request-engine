    ...
    op.execute(
        ...
        BEGIN
            ...
            IF p_invitation_id IS NULL
               ...
               OR p_idempotency_key_digest !~ '^[0-9a-f]{64}            p_invitation_id uuid,
            p_principal_id uuid,
            p_binding_id uuid,
            p_idempotency_key_digest text,
            p_intent_digest text
...
def downgrade() -> None:
    raise RuntimeError("Platform Owner invitation history is append-preserving; roll forward")
            ...
            intent_digest text
                CHECK (
                    intent_digest IS NULL
                    OR intent_digest ~ '^[0-9a-f]{64}        );
        ALTER TABLE request_engine.platform_owner_invitation_facts
            OWNER TO request_engine_schema_owner;
        REVOKE ALL ON request_engine.platform_owner_invitation_facts FROM PUBLIC;
        CREATE TRIGGER platform_owner_invitation_facts_append_only
...
def downgrade() -> None:
    raise RuntimeError("Platform Owner invitation history is append-preserving; roll forward")
            ...
            created_at timestamptz NOT NULL DEFAULT clock_timestamp(),
            CONSTRAINT platform_owner_invitation_fact_idempotency_uq
...
def downgrade() -> None:
    raise RuntimeError("Platform Owner invitation history is append-preserving; roll forward")
               ...
               OR p_intent_digest !~ '^[0-9a-f]{64}            p_invitation_id uuid,
            p_principal_id uuid,
            p_binding_id uuid,
            p_idempotency_key_digest text,
            p_intent_digest text
