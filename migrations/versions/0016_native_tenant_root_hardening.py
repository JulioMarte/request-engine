"""Harden Native tenant-root bootstrap identity and Party provenance.

Revision ID: 0016_native_root_hardening
Revises: 0015_native_tenant_root
...
revision: str = "0016_native_root_hardening"
down_revision: str | Sequence[str] | None = "0015_native_tenant_root"
...
def upgrade() -> None:
    ...
    op.execute(
        "GRANT INSERT (organization_id, party_id, revision, change_kind, display_name, active, "
        "state, actor_principal_id, attributed_operator_principal_id, source_kind, platform) "
        f"ON request_engine.party_identity_revisions TO {_DEFINER_ROLE}"
    )
    op.execute(
        """
        ...
        BEGIN
            ...
            INSERT INTO request_engine.party_identity_revisions (
                ...
                v_creator_id, v_creator_id, 'operator', 'request_engine'
            );
...
def downgrade() -> None:
    op.execute(
        "REVOKE INSERT (organization_id, party_id, revision, change_kind, display_name, active, "
        "state, actor_principal_id, attributed_operator_principal_id, source_kind, platform) "
        f"ON request_engine.party_identity_revisions FROM {_DEFINER_ROLE}"
    )
    op.execute(
