"""Separate Platform provisioning provenance from tenant-local Party actors.

Revision ID: 0017_native_root_provenance
Revises: 0016_native_root_hardening
Create Date: 2026-09-08

Party actor columns are tenant-local by composite foreign key. A Platform
provisioner therefore cannot truthfully populate them without becoming a tenant
Principal. Platform provenance remains in the dedicated provisioning facts.
"""
...
revision: str = "0017_native_root_provenance"
down_revision: str | Sequence[str] | None = "0016_native_root_hardening"
...
def upgrade() -> None:
    op.execute(
        "REVOKE INSERT (created_by_principal_id, source_kind, platform) "
        f"ON request_engine.parties FROM {_DEFINER_ROLE}"
    )
    op.execute(
        "REVOKE INSERT (actor_principal_id, attributed_operator_principal_id, source_kind, "
        f"platform) ON request_engine.party_identity_revisions FROM {_DEFINER_ROLE}"
    )
    op.execute(
        """
        CREATE OR REPLACE FUNCTION request_platform.provision_native_organization_root(
...
def downgrade() -> None:
    # The pre-hardening implementation is intentionally not restored. This
    # migration only narrows privileges and corrects provenance semantics.
    pass
