...
LEFT JOIN LATERAL (
    SELECT hold.id, hold.condition_kind, hold.until_at, hold.event_key, hold.reason
     ...
     WHERE hold.organization_id = r.organization_id
       AND hold.queue_entry_id = qe.id
       AND hold.released_at IS NULL
       AND (
           hold.condition_kind <> 'until_time'
           OR hold.until_at > clock_timestamp()
       )
...
LEFT JOIN LATERAL (
    SELECT skip.reason
     ...
     WHERE skip.organization_id = r.organization_id
       AND skip.queue_entry_id = qe.id
