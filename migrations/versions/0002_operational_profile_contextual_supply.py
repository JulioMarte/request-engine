...
ALTER TABLE request_engine.organizations
    ...
    ADD CONSTRAINT organizations_legal_name_ck CHECK (legal_name IS NULL OR btrim(legal_name) <> ''),
    ADD CONSTRAINT organizations_default_timezone_ck CHECK (default_timezone IS NULL OR btrim(default_timezone) <> ''),
    ADD CONSTRAINT organizations_default_locale_ck CHECK (default_locale IS NULL OR btrim(default_locale) <> ''),
    ADD CONSTRAINT organizations_default_currency_ck CHECK (default_currency IS NULL OR default_currency ~ '^[A-Z]{3}$'),
    ADD CONSTRAINT organizations_operational_status_ck CHECK (operational_status IN ('active', 'inactive'));
...
ALTER TABLE request_engine.locations
    ...
    ADD CONSTRAINT locations_coordinate_pair_ck CHECK ((latitude IS NULL) = (longitude IS NULL)),
    ADD CONSTRAINT locations_latitude_ck CHECK (latitude IS NULL OR latitude BETWEEN -90 AND 90),
    ADD CONSTRAINT locations_longitude_ck CHECK (longitude IS NULL OR longitude BETWEEN -180 AND 180),
    ADD CONSTRAINT locations_country_code_ck CHECK (country_code IS NULL OR country_code ~ '^[A-Z]{2}$'),
    ADD CONSTRAINT locations_address_line1_ck CHECK (address_line1 IS NULL OR btrim(address_line1) <> ''),
    ADD CONSTRAINT locations_operational_revision_ck CHECK (operational_revision > 0);
...
CREATE TABLE request_engine.location_public_contact_endpoints (
    ...
    FOREIGN KEY (organization_id, location_id) REFERENCES request_engine.locations (organization_id, id),
    CHECK (channel IN ('phone', 'whatsapp', 'email')),
    CHECK (normalized_value <> ''),
    CHECK (label IS NULL OR btrim(label) <> '')
);
...
CREATE INDEX location_operational_hours_lookup_idx
    ON request_engine.location_operational_hours (organization_id, location_id, weekday, active);
...
CREATE TABLE request_engine.location_hours_exceptions (
    ...
    CONSTRAINT location_hours_exceptions_no_active_overlap
        EXCLUDE USING gist (organization_id WITH =, location_id WITH =, during WITH &&) WHERE (active)
);
...
CREATE TABLE request_engine.resource_location_assignments (
    ...
    updated_at timestamptz NOT NULL DEFAULT clock_timestamp(),
    UNIQUE (organization_id, id),
    FOREIGN KEY (organization_id, resource_id) REFERENCES request_engine.resources (organization_id, id),
    FOREIGN KEY (organization_id, location_id) REFERENCES request_engine.locations (organization_id, id),
...
CREATE INDEX resource_location_assignments_resource_idx
    ON request_engine.resource_location_assignments (organization_id, resource_id, status);
CREATE INDEX resource_location_assignments_location_idx
    ON request_engine.resource_location_assignments (organization_id, location_id, status);
...
CREATE INDEX booking_context_terms_offering_idx
    ON request_engine.booking_context_terms (organization_id, offering_version_id, active);
...
CREATE INDEX capacity_claims_resource_location_assignment_idx
    ON request_engine.capacity_claims (organization_id, resource_location_assignment_id)
...
CREATE FUNCTION request_engine.bump_resource_from_assignment()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
...
FOR EACH ROW EXECUTE FUNCTION request_engine.bump_resource_from_assignment();

CREATE FUNCTION request_engine.bump_resource_from_assignment_child()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
DECLARE
...
FOR EACH ROW EXECUTE FUNCTION request_engine.bump_resource_from_assignment_child();
...
BEGIN
    ...
    SELECT r.capacity_model, r.capacity_units, r.active, r.location_id
     ...
     FOR UPDATE;
    IF NOT FOUND THEN
        RAISE EXCEPTION 'Resource % does not exist for capacity claim', NEW.resource_id USING ERRCODE = '23503';
    END IF;
    IF NOT v_resource_active THEN
        RAISE EXCEPTION 'Resource % is inactive', NEW.resource_id USING ERRCODE = '23514';
    END IF;
    ...
    IF NEW.reservation_id IS NOT NULL THEN
        SELECT r.offering_version_id, r.during, r.location_id
         ...
         WHERE r.organization_id = NEW.organization_id AND r.id = NEW.reservation_id
    ...
    ELSE
        IF NEW.hold_id IS NULL THEN
            RAISE EXCEPTION 'active hold claim requires CapacityHold' USING ERRCODE = '23514';
        END IF;
        SELECT h.offering_version_id, h.during, h.location_id
          INTO v_owner_offering_version, v_owner_during, v_owner_location
          FROM request_engine.capacity_holds h
         WHERE h.organization_id = NEW.organization_id AND h.id = NEW.hold_id
    ...
    IF NEW.during <> v_owner_during THEN
        RAISE EXCEPTION 'CapacityClaim interval must equal its Hold/Reservation interval' USING ERRCODE = '23514';
    END IF;
    ...
    IF NEW.resource_location_assignment_id IS NOT NULL THEN
        SELECT a.resource_id, a.location_id, a.effective_during, a.status
         ...
         WHERE a.organization_id = NEW.organization_id AND a.id = NEW.resource_location_assignment_id;
        IF NOT FOUND THEN
        ...
        IF v_assignment_status <> 'active' THEN
            RAISE EXCEPTION 'CapacityClaim ResourceLocationAssignment is not active' USING ERRCODE = '23514';
        END IF;
    ...
    SELECT rr.offering_version_id, rr.capability_id, rr.quantity
     ...
     WHERE rr.organization_id = NEW.organization_id AND rr.id = NEW.requirement_id;
    ...
    IF NOT EXISTS (
        SELECT 1 FROM request_engine.resource_capability_assignments a
         WHERE a.organization_id = NEW.organization_id
           AND a.resource_id = NEW.resource_id AND a.capability_id = v_required_capability
    ) THEN
        RAISE EXCEPTION 'Resource % does not satisfy required capability', NEW.resource_id USING ERRCODE = '23514';
    END IF;
    ...
    SELECT COALESCE(sum(c.quantity), 0), count(*)
     ...
     WHERE c.organization_id = NEW.organization_id
       ...
       AND (
           ...
           OR (c.reservation_id IS NULL AND h.status = 'active' AND h.expires_at > clock_timestamp())
       );
    IF v_capacity_model = 'exclusive' AND v_other_count > 0 THEN
        RAISE EXCEPTION 'exclusive Resource % has overlapping live capacity', NEW.resource_id USING ERRCODE = '23P01';
    END IF;
    IF v_capacity_model = 'units' AND v_other_quantity + NEW.quantity > v_capacity_units THEN
        RAISE EXCEPTION 'Resource % capacity exceeded: requested %, live %, capacity %',
            NEW.resource_id, NEW.quantity, v_other_quantity, v_capacity_units USING ERRCODE = '23P01';
    END IF;
    RETURN NEW;
END
$function$;
