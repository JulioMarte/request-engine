...
Every party mutation appends one `party_identity_revisions` row inside its
existing transaction. The revision number is the NEW value of
`parties.identity_revision`: registration creates the Party at revision 1
and every later mutation increments the cursor atomically with the ledger
insert (`UPDATE ... RETURNING`). The recorded snapshot is the full
post-mutation identity state, read from the authoritative rows in the same
transaction.
"""
...
_INSERT_REVISION_SQL = text(
    ...
    " (organization_id, party_id, revision, change_kind, display_name, active,"
    "  state, actor_principal_id, attributed_operator_principal_id,"
    "  source_kind, platform)"
    " VALUES (:organization_id, :party_id, :revision, :change_kind, :display_name,"
    "  :active, CAST(:state AS jsonb), :actor_principal_id,"
    "  :attributed_operator_principal_id, :source_kind, :platform)"
    " RETURNING revision"
)
...
async def record_party_revision(
    ...
    *,
    ...
    """Append one ledger revision with the post-mutation identity snapshot."""
