...
from request_engine.modules.queue.application.commands.operator_select import (
    OperatorSelectCommand,
    OperatorSelectExecutor,
)
from request_engine.modules.queue.application.commands.skip_queue_head import (
    SkipQueueHeadCommand,
    SkipQueueHeadExecutor,
)
...
def create_same_day_dispatch_router(
    *,
    ...
    async def operator_select_entry(
        queue_id: UUID,
        queue_entry_id: UUID,
        ...
        entry = await operator_select_executor.operator_select(
            OperatorSelectCommand(
    ...
    async def skip_head(
        ...
        result = await skip_executor.skip_queue_head(
            SkipQueueHeadCommand(
                organization_id=current.organization_id,
                principal_id=current.principal_id,
                queue_id=queue_id,
