...
async def update_workload_classification(
    session_factory: SessionFactory,
    command: UpdateWorkloadClassificationCommand,
    ...
    async with tenant_transaction(session_factory, command.organization_id) as session:
        ...
        if not cast(bool, row["active"]):
            raise WorkloadClassificationInactive(command.workload_id)
        if cast(str, row["display_name"]) == command.display_name.strip():
            result = workload_from_row(row)
        else:
            updated = await session.execute(
                text(
                    "UPDATE request_engine.operational_workload_classifications "
                    "SET display_name=btrim(:display_name),revision=revision+1,"
            ...
            await record_workload_fact(
                ...
                workload=result,
                event_type="workload.classification_updated.v1",
            )
