        ...
        try:
            async with tenant_transaction(self._session_factory, command.organization_id) as session:
                ...
                if location_exists is None:
                    raise ContextualConfigurationConflict("Location is missing, inactive, or foreign")
                if command.capability_ids:
                    count = cast(
                        ...
                        (
                            await session.execute(
                                text(
                                    """
                                    SELECT count(*) FROM request_engine.resource_capabilities
                                    WHERE organization_id = :organization_id
                    ...
                    if count != len(command.capability_ids):
                        raise ContextualConfigurationConflict("Resource capabilities are missing or foreign")
                row = (
                    (
                        await session.execute(
                            text(
                ...
                state = ResourceBootstrapState(
                    ...
                    capacity_model=cast(object, row["capacity_model"]),  # type: ignore[arg-type]
            ...
            if getattr(exc.orig, "sqlstate", None) == "23505":
                raise ContextualConfigurationConflict("Resource conflicts with tenant configuration") from None
            raise
...
def _state_from_json(value: dict[str, object]) -> ResourceBootstrapState:
    return ResourceBootstrapState(
        ...
        capacity_model=cast(object, value["capacity_model"]),  # type: ignore[arg-type]
