...
async def supersede_and_insert(
    session: AsyncSession, command: RecordArrivalEstimateCommand
) -> RowMapping:
    await session.execute(
        text(
            """
            ...
            await session.execute(
                text(
                    ...
                    INSERT INTO request_engine.reservation_arrival_estimates (
                        organization_id, reservation_id, estimated_arrival_at,
                        source_kind, asserted_by_principal_id
                    ) VALUES (
                        :organization_id, :reservation_id, :estimated_arrival_at,
                        :source_kind, :principal_id
                    )
