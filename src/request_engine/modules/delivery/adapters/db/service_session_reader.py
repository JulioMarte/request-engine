...
from request_engine.modules.delivery.contracts.service_session import ServiceSession, ServiceSessionStatus
...
class PostgresServiceSessionReader:
    ...
    async def get(self, organization_id: UUID, service_session_id: UUID) -> ServiceSession:
            ...
            row = (
                (
                    await session.execute(
                        text(
                            """
                            SELECT service_session_id AS id, queue_entry_id, resource_id, location_id,
        ...
        return ServiceSession(
            id=cast(UUID, row["id"]), queue_entry_id=cast(UUID, row["queue_entry_id"]),
            resource_id=cast(UUID, row["resource_id"]), location_id=cast(UUID, row["location_id"]),
