...
def parse_delivery_policy(value: dict[str, object]) -> DeliveryPolicy:
    ...
    if (
        isinstance(raw_reconcile, bool)
        or not isinstance(raw_reconcile, int)
        or raw_reconcile < 30
        or raw_reconcile > 86400
    ):
        raise DeliveryConfigurationError(
            "channel_policy.reconcile_after_seconds must be between 30 and 86400"
        )
    ...
    return DeliveryPolicy(
        routes=tuple(routes),
        reconcile_after_seconds=raw_reconcile,
    )
