...
class AgentPolicyActorResolver:
    ...
    Missing policies and unmarked routes fail closed, except the explicitly
    identified read-only self operation catalog after the same fresh policy read.
    """
    ...
    async def resolve_actor(self, request: Request) -> ActorContext:
        ...
        metadata = getattr(route, "openapi_extra", None)
        if (
            capability_key is None
            and request.scope.get("method") == "GET"
            and getattr(route, "operation_id", None) == "operation_catalog_list_authorized"
            and isinstance(metadata, dict)
            and cast(dict[str, object], metadata).get("x-request-engine-discovery") is True
        ):
        ...
        if not agent_risk_permitted(risk, policy.risk_ceiling):
